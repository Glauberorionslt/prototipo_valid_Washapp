...
class Company(Base):
    ...
    contract_status: Mapped[str] = mapped_column(
        String(20), default=ContractStatus.ACTIVE.value)
    ...
    team_members: Mapped[list["TeamMember"]
                         ] = relationship(back_populates="company")
    team_cost_entries: Mapped[list["TeamCostEntry"]
                              ] = relationship(back_populates="company")
    operational_cost_types: Mapped[list["OperationalCostType"]] = relationship(
        back_populates="company")
...
class User(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    company_id: Mapped[int | None] = mapped_column(ForeignKey(
        "companies.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    access_key_id: Mapped[int | None] = mapped_column(
        ForeignKey("access_keys.id", ondelete="SET NULL"), nullable=True)
    activated_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True)
    manager_password_hash: Mapped[str | None] = mapped_column(
...
class AccessKey(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    key_token: Mapped[str] = mapped_column(
        String(255), unique=True, index=True)
    label: Mapped[str | None] = mapped_column(String(255), nullable=True)
    status: Mapped[str] = mapped_column(
        String(20), default=AccessKeyStatus.ACTIVE.value)
...
class Customer(Base):
    ...
    __table_args__ = (UniqueConstraint("company_id", "plate",
                      name="uq_customer_company_plate"),)
...
class Order(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(20), default=OrderStatus.AGUARDANDO.value, index=True)
...
class TeamMember(Base):
    ...
    __table_args__ = (UniqueConstraint("company_id", "name",
                      name="uq_team_member_company_name"),)
...
class TeamCostEntry(Base):
    ...
    member_id: Mapped[int] = mapped_column(ForeignKey(
        "team_members.id", ondelete="CASCADE"), index=True)
...
class OperationalCostType(Base):
    ...
    __table_args__ = (UniqueConstraint("company_id", "name",
                      name="uq_operational_cost_type_company_name"),)
    ...
    entries: Mapped[list["OperationalCostEntry"]] = relationship(
...
class OperationalCostEntry(Base):
    ...
    cost_type_id: Mapped[int] = mapped_column(ForeignKey(
        "operational_cost_types.id", ondelete="CASCADE"), index=True)
    entry_date: Mapped[date] = mapped_column(Date, index=True)
    amount: Mapped[float] = mapped_column(Numeric(10, 2))
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=datetime.utcnow)
